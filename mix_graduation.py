#!/usr/bin/env python3
"""Mix the entire Graduation album by Kanye West — generates transitions between every consecutive track."""
import subprocess
import sys
import shutil
import time
...
OUTPUT_DIR = Path("graduation_mix")
OUTPUT_DIR.mkdir(exist_ok=True)

# Graduation tracklist in album order
TRACKS = [
...
def short_name(track: str) -> str:
    ...
    return track.split(" Kanye")[0].lower().replace(" ", "_").replace(".", "").replace("'", "").replace("?", "").replace("!", "")

def main():
    total = len(TRACKS) - 1
    print("=" * 60)
    print(f"🎓 GRADUATION FULL ALBUM MIX — {total} transitions")
    print("=" * 60)
